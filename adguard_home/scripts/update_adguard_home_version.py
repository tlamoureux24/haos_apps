#!/usr/bin/env python3
"""Update the pinned stable AdGuard Home version and package metadata."""
...
def main() -> int:
    ...
    if current == upstream:
        print(f"AdGuard Home {upstream} is already pinned")
        return 0
    ...
    if tuple(map(int, upstream.split("."))) < tuple(map(int, current.split("."))):
        print(f"Refusing to downgrade AdGuard Home {current} -> {upstream}", file=sys.stderr)
        return 2
    ...
    replace_once(
        ...
        f'ARG ADGUARD_HOME_VERSION="{upstream}"',
    )
    replace_once(
        ROOT / "Dockerfile",
        r'^ARG BUILD_VERSION="[^"]+"$',
    ...
    entry = (
        f"## {package} - {date}\n\n"
        f"- Update the official AdGuard Home image from {current} to {upstream}.\n"
    ...
    print(f"Updated AdGuard Home {current} -> {upstream}; app version {package}")
    return 0
