...
> [!WARNING]
> **Dans l'assistant initial, remplacez obligatoirement le port web `80` par
> `3000`.** Sinon, son dernier bouton pointera vers le port 80 non publié au
> lieu de l'interface d'administration de l'App.
...
L'App remplace également `80` par `3000` comme garde-fou avant le redémarrage
final, mais elle ne peut pas corriger le lien déjà généré par le dernier bouton
de l'assistant.
...
> [!WARNING]
> **In the initial wizard, you must replace web port `80` with `3000`.**
> Otherwise, its final button points to unpublished port 80 instead of the
> App's administration interface.
...
The App also changes `80` to `3000` as a safety net before the final restart,
but it cannot correct the link already generated by the wizard's final button.
