...
def main() -> int:
    ...
    for text in ('slug: "agent_execution_plane"', 'version: "0.5.3"', "ingress_port: 8099", "  8098/tcp: null", "apparmor: true", "tmpfs: true"):
        if text not in config: raise RuntimeError(f"Missing metadata invariant: {text}")
    if '__version__ = "0.5.3"' not in package: raise RuntimeError("Version sources differ")
