...
def main() -> int:
    ...
    for text in ('slug: "agent_execution_plane"', 'version: "1.1.1"', "stage: stable", "ingress_port: 8099", "  8098/tcp: null", "apparmor: true", "tmpfs: true"):
        if text not in config: raise RuntimeError(f"Missing metadata invariant: {text}")
    if '__version__ = "1.1.1"' not in package: raise RuntimeError("Version sources differ")
