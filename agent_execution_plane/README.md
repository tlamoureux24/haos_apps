...
Agent Execution Plane is a standalone-capable model reasoning and execution engine. It applies administrator model priority and the common fallback/no-replay rules to exactly the MCP operational capability envelope supplied by the current source.
...
The standalone API defaults to HTTPS with a persistent self-generated
certificate. **Transport & TLS** exposes listener state, fingerprint, expiry,
and regeneration. External files are selected by filenames relative to `/ssl`,
for example `agent-suite-cert.pem` and `agent-suite-key.pem`. See the
[bilingual TLS operations guide](../TLS.md).

| App option | Values | Default | Meaning |
|---|---|---|---|
| `public_transport` | `http`, `https` | `https` | Standalone API transport |
...
```bash
curl -X POST 'https://HOME_ASSISTANT_HOST:AEP_PORT/api/v1/execute' \
...
curl -H 'Authorization: Bearer <AEP_STANDALONE_TOKEN>' \
  'https://HOME_ASSISTANT_HOST:AEP_PORT/api/v1/executions/<EXECUTION_ID>'
```
...
curl -X POST -H 'Authorization: Bearer <AEP_STANDALONE_TOKEN>' \
  'https://HOME_ASSISTANT_HOST:AEP_PORT/api/v1/executions/<EXECUTION_ID>/ack'
```
...
Map `8098/tcp` only when a standalone caller needs it. Self-generated HTTPS is
the default. HTTP remains available for compatibility, is unencrypted, and
produces an explicit warning. See the [TLS operations guide](../TLS.md).
...
4. In AEP **Control Plane**, enter ACP's full endpoint, for example `https://HOME_ASSISTANT_IP:8098/mcp`.
5. Paste the worker credential and save.

When ACP uses its autogenerated certificate, also copy its independently
verified SHA-256 fingerprint into AEP. Leave the field empty for a certificate
validated normally by the system CA store. AEP's standalone API defaults to
HTTPS with its own persistent autogenerated certificate; its fingerprint and
regeneration action are available in the Ingress administration UI.
...
curl -X POST 'https://HOME_ASSISTANT_IP:AEP_PORT/api/v1/execute' \
