...
FAMILIES = {"ollama_compatible", "openai_compatible"}
...
class ModelStore:
    def __init__(self, database: Path, private_dir: Path):
    ...
    def save(self, candidate: Candidate, model_id: str | None = None) -> tuple[dict[str, object] | None, ProviderCheck]:
        ...
        credential = candidate.credential if candidate.replace_credential else (decrypt(self.key, previous["encrypted_credential"]) if previous else candidate.credential)
        result = check(candidate.provider_family, candidate.base_url, candidate.provider_model, credential, explicit=True)
        if result.state != "available": return None, result
        encrypted = encrypt(self.key, credential) if credential else None
        timestamp = now_iso()
        with connect(self.database) as db:
            if previous:
                db.execute("UPDATE models SET display_name=?,provider_family=?,base_url=?,provider_model=?,encrypted_credential=?,enabled=?,timeout_minutes=?,technical_state=?,diagnostic_code=?,checked_at=?,updated_at=? WHERE id=?", (candidate.display_name.strip(), candidate.provider_family, candidate.base_url.rstrip('/'), candidate.provider_model.strip(), encrypted, int(candidate.enabled), candidate.timeout_minutes, result.state, result.code, timestamp, timestamp, model_id))
            else:
                model_id = str(uuid4())
                priority = db.execute("SELECT coalesce(max(priority),0)+1 FROM models").fetchone()[0]
                db.execute("INSERT INTO models VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (model_id, candidate.display_name.strip(), candidate.provider_family, candidate.base_url.rstrip('/'), candidate.provider_model.strip(), encrypted, int(candidate.enabled), priority, candidate.timeout_minutes, result.state, result.code, timestamp, timestamp, timestamp))
        record_activity(self.database, "model_updated" if previous else "model_created", "configuration", "success")
        if previous and candidate.replace_credential:
    ...
    def refresh_health(self) -> None:
        ...
        for row in rows:
            ...
            result = check(row["provider_family"], row["base_url"], row["provider_model"], credential, explicit=False)
