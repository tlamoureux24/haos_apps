...
Rsync Manager est une App Home Assistant pour creer, planifier, tester et lancer des synchronisations `rsync` depuis une interface web Ingress.
...
L'App utilise Ingress : elle s'ouvre directement dans Home Assistant, sans port publie sur le reseau local.
...
Le bouton `Actualiser` recharge les jobs et les statuts depuis l'App. Il est utile si un job cron s'est execute pendant que l'interface etait ouverte.
...
En mode `Local`, indiquez un chemin visible depuis le conteneur de l'App.
...
Les chemins disponibles dependent des dossiers exposes par Home Assistant dans la configuration de l'App.
...
Les options techniques rsync et CIFS ne sont plus exposees dans l'interface. L'App applique un profil integre pour reduire les erreurs de configuration.

### Profil CIFS

Pour les montages SMB/CIFS, l'App utilise:
...
Pour `Simuler` et `Lancer`, l'App utilise:
...
Apres chaque ajout, modification, suppression ou import de jobs, l'App:
...
Les certificats TLS du serveur SMTP sont verifies avec le magasin de certificats systeme. Un certificat autosigne ou non reconnu est donc refuse.
...
Les exports contiennent les mots de passe SMTP et SMB/CIFS en clair. Conservez ces fichiers dans un emplacement sûr.

Lors d'un import de jobs, l'App normalise automatiquement:
...
Ces donnees sont incluses dans les sauvegardes Home Assistant. L'App utilise une sauvegarde a froid : elle est arretee pendant la copie afin de conserver un etat coherent.
...
Rsync est installe depuis le depot Alpine de l'image de base Home Assistant courante. Un workflow GitHub quotidien construit cette image exacte et controle la version du paquet installe.

Lorsqu'une nouvelle version du paquet est disponible, le workflow :

1. met a jour la version de paquet enregistree ;
2. incremente la version de l'App ;
3. valide, construit et teste l'interface ;
4. publie la modification seulement si tous les controles reussissent.

Home Assistant propose ensuite la mise a jour normalement. Son installation reste volontaire, sauf si l'utilisateur active lui-meme les mises a jour automatiques dans Home Assistant.

## Logs De L'App

Les logs utiles sont visibles dans le journal de l'App Home Assistant et, pour chaque job, dans `Voir dernier log`.
...
L'App applique deja un profil SMB/CIFS integre. Si l'erreur persiste, verifiez surtout les droits du compte SMB, le chemin destination et les permissions du partage cote serveur.
