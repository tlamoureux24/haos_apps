# Agent Gateway — MCP compatibility / Compatibilité MCP
...
actuelle d’Agent Gateway. Elle distingue les fonctions prises en charge par le
SDK MCP épinglé de celles directement exercées par la CI d’Agent Gateway ; elle
ne constitue pas une certification de conformité complète au protocole MCP.

La version actuelle d’Agent Gateway utilise `mcp==1.28.1` pour son serveur MCP
public et pour ses connexions MCP amont.

### Transports et rôles

| Rôle Agent Gateway | Transport pris en charge | Authentification | Fonctions MCP utilisées |
| --- | --- | --- | --- |
| Serveur public pour les agents | Streamable HTTP sur `/mcp` | Bearer Agent Gateway obligatoire | `initialize`, `tools/list`, `tools/call`, notifications `tools/list_changed` |
...
MCP ne sont pas pris en charge. Agent Gateway ne proxifie pas les ressources,
...
Le SDK Python MCP 1.28.1 utilisé par Agent Gateway annonce les révisions prises
en charge suivantes :
...
Sa révision la plus récente est `2025-11-25`. Agent Gateway délègue la
négociation `initialize` à ce SDK et n’ajoute pas de mécanisme de négociation
propriétaire.

La CI d’Agent Gateway exerce directement son listener public avec une requête
`initialize` en `2025-06-18`, vérifie qu’un appel non authentifié est refusé et
qu’un appel authentifié obtient bien une réponse du serveur Agent Gateway. Les
...
utiliser un Bearer fourni explicitement par l’administrateur ; Agent Gateway ne
...
Schema Draft 2020-12 fail-closed d’Agent Gateway, avec références locales
...
Agent Gateway ne revendique actuellement ni OAuth MCP, ni connecteurs `stdio`,
...
Sources d’implémentation : `requirements.txt`, `src/agent_gateway/connectors.py`,
`src/agent_gateway/mcp_api.py`, `src/agent_gateway/json_contracts.py` et le smoke
test de `.github/workflows/agent-gateway-validate.yml`.
...
stable Agent Gateway branch. It distinguishes capabilities inherited from the
pinned MCP SDK from behavior directly exercised by Agent Gateway CI; it is not a
claim of full MCP conformance certification.

The current Agent Gateway release uses `mcp==1.28.1` for both its public MCP
server and its upstream MCP client connections.

### Transports and roles

| Agent Gateway role | Supported transport | Authentication | MCP operations used |
| --- | --- | --- | --- |
| Public server for agents | Streamable HTTP on `/mcp` | Agent Gateway Bearer required | `initialize`, `tools/list`, `tools/call`, `tools/list_changed` notifications |
...
supported. Agent Gateway does not proxy upstream resources, prompts, roots,
...
The MCP Python SDK 1.28.1 pinned by Agent Gateway advertises support for these
protocol revisions:
...
Its latest revision is `2025-11-25`. Agent Gateway delegates `initialize`
negotiation to that SDK and adds no proprietary negotiation layer.

Agent Gateway CI directly exercises the public listener with a `2025-06-18`
`initialize` request, verifies that an unauthenticated request is rejected, and
that an authenticated request receives an Agent Gateway server response. The
other revisions above are therefore supported by the pinned MCP dependency but
are not claimed as individually covered by an Agent Gateway conformance test.
...
supplied by the administrator; Agent Gateway does not implement connector OAuth
in this release.

Discovery is bounded to 200 tools per connector. Each tool input schema is
limited to 16 KiB encoded and must be admitted by Agent Gateway's fail-closed
...
Agent Gateway currently claims no MCP OAuth, `stdio` connectors, legacy SSE
...
Implementation evidence: `requirements.txt`, `src/agent_gateway/connectors.py`,
`src/agent_gateway/mcp_api.py`, `src/agent_gateway/json_contracts.py`, and the
smoke test in `.github/workflows/agent-gateway-validate.yml`.
