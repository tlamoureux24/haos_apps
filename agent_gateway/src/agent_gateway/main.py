...
<section id="triggers" class="view"><div class="pagehead"><h1>Déclencheurs</h1><p>Associez explicitement une source et un type d’événement à une tâche.</p></div><div class="workspace"><section class="card"><div class="cardhead"><div><h2>Nouveau déclencheur</h2><p>La source ne pourra pas choisir une autre tâche dans sa requête.</p></div></div><form id="mapping-create"><label>Nom<input name="display_name" maxlength="120" placeholder="Ex. Alerte supervision" required></label><label>Source d’événements<select id="mapping-source" name="source_identity_id" required></select></label><label>Type d’événement<input name="event_type" maxlength="120" pattern="[a-z][a-z0-9_.-]*" placeholder="service.alert" required></label><label>Tâche<select id="mapping-task" name="task_id" required></select></label><label>Cooldown<select name="cooldown_minutes"><option value="0">Aucun</option><option value="5">5 minutes</option><option value="15" selected>15 minutes</option><option value="30">30 minutes</option><option value="60">1 heure</option><option value="360">6 heures</option><option value="1440">24 heures</option></select></label><button class="primary" type="submit">Créer le déclencheur</button><p id="mapping-message" class="error"></p></form></section><section class="card identities"><div class="cardhead"><div><h2>Déclencheurs configurés</h2><p>Les événements supprimés par le cooldown restent visibles et audités.</p></div><span id="mapping-count" class="count">–</span></div><div id="mapping-list"><p class="loading">Chargement…</p></div></section></div></section>
<section id="schedules" class="view"><div class="pagehead"><h1>Planifications</h1><p>Créez automatiquement des exécutions récurrentes à partir de tâches prêtes.</p></div><div class="workspace"><section class="card"><div class="cardhead"><div><h2>Nouvelle planification</h2><p>Les occurrences manquées pendant un arrêt ne sont pas rejouées en rafale.</p></div></div><form id="schedule-create"><label>Nom<input name="display_name" maxlength="120" placeholder="Ex. Diagnostic quotidien" required></label><label>Tâche<select id="schedule-task" name="task_id" required></select></label><label>Fréquence<select name="interval_minutes"><option value="5">Toutes les 5 minutes</option><option value="15">Toutes les 15 minutes</option><option value="30">Toutes les 30 minutes</option><option value="60" selected>Toutes les heures</option><option value="360">Toutes les 6 heures</option><option value="1440">Tous les jours</option><option value="10080">Toutes les semaines</option></select></label><button class="primary" type="submit">Créer la planification</button><p id="schedule-message" class="error"></p></form></section><section class="card identities"><div class="cardhead"><div><h2>Planifications configurées</h2><p>Une tâche indisponible est ignorée sans créer de travail invalide.</p></div><span id="schedule-count" class="count">–</span></div><div id="schedule-list"><p class="loading">Chargement…</p></div></section></div></section>
