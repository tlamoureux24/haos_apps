...
def main() -> int:
    ...
    if '__version__ = "0.46.2"' not in package:
    ...
    if launcher.count("--log-config /app/src/agent_gateway/uvicorn_logging.json") != 2:
        raise RuntimeError("Both Agent Gateway listeners must use the timestamped logging configuration")
