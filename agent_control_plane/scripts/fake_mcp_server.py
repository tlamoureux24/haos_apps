#!/usr/bin/env python3
"""Small read-only MCP server for Agent Control Plane multi-connector acceptance tests."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--server-marker",
        default="agent-control-plane-fake-mcp",
...
def main() -> None:
    ...
    server = FastMCP(
        f"Agent Control Plane fake MCP ({args.server_marker})",
    ...
    def fake_ha_get_addon(slug: str | None = None, source: str = "installed") -> dict[str, object]:
        return {
            ...
            "read_only": True,
            "message": "Synthetic response from an Agent Control Plane acceptance connector; no Home Assistant call occurred.",
    ...
    def echo_probe(message: str = "Agent Control Plane test") -> dict[str, object]:
        return {
            "server_marker": args.server_marker,
