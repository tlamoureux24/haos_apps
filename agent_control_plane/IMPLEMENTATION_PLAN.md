# Agent Control Plane — revised implementation plan

This plan replaces the original HA-MCP/Gatus-centred sequence. Agent Control Plane is
...
Agent Control Plane has four independent concepts:

1. **Client identities** authenticate people, agents, event sources and internal
   schedulers to Agent Control Plane.
...
Agent Control Plane follows a deny-by-default capability model comparable to an
...
Agent Control Plane does not infer authorization from semantic labels such as `read`,
...
- identity/credential management and control-plane permissions;
...
Agent Control Plane without claiming to revoke the old token at the upstream server.
...
- composable control-plane permissions and deny-by-default enforcement;
...
- a generic test MCP server can be added without changing Agent Control Plane code;
...
- no task can select connector administration, secret or raw passthrough
  operations supplied by Agent Control Plane itself;
...
- connector credentials never cross the Agent Control Plane boundary;
...
- making Agent Control Plane an OAuth authorization server;
...
Gateway. Agent Control Plane remains model-neutral, exposes its governed MCP surface
