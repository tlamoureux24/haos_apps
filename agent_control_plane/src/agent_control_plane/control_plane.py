...
from agent_control_plane.fixed_arguments import (
...
from agent_control_plane.security import (
    IssuedCredential,
    issue_credential,
...
class ControlPlane:
    ...
    def enqueue_manual_task(
        ...
        with connect(self.database_path) as connection:
            ...
            connection.execute(
                ...
                (policy_revision_id, policy_id, 1, canonical_json({"allow": {"control_plane_actions": [], "capabilities": []}, "deny": {"control_plane_actions": [], "capabilities": []}}), now),
            )
            connection.execute(
    ...
    def create_identity(
        ...
        policy = {
            "schema_version": 1,
            "allow": {"control_plane_actions": list(normalized_actions), "capabilities": []},
            "deny": {"control_plane_actions": [], "capabilities": []},
            ...
            identities.append(
                {
                    ...
                    "control_plane_actions": list(
                        validate_actions(policy.get("allow", {}).get("control_plane_actions", []))
                    ),
                }
            )
        return identities
    ...
    def _metadata_json(connection: sqlite3.Connection, key: str) -> dict[str, object] | None:
        row = connection.execute("SELECT value FROM control_plane_metadata WHERE key=?", (key,)).fetchone()
        if row is None:
            return None
        try:
    ...
    def _set_metadata_json(connection: sqlite3.Connection, key: str, value: dict[str, object]) -> None:
        connection.execute(
            "INSERT INTO control_plane_metadata(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value",
    ...
    def _audit_checkpoint_is_authenticated(self, checkpoint: dict[str, object]) -> bool:
        ...
        material = canonical_json({
            "domain": "agent-control-plane-audit-checkpoint-v1",
    ...
    def retention_status(self) -> dict[str, object]:
        with connect(self.database_path) as connection:
            values = {row["key"]: row["value"] for row in connection.execute(
                "SELECT key,value FROM control_plane_metadata WHERE key IN ('retention_days','retention_batch_size','retention_automatic','retention_last_run_at')"
            ).fetchall()}
    ...
    def set_retention_policy(self, retention_days: int, batch_size: int, automatic: bool, correlation_id: str) -> None:
        ...
        with connect(self.database_path) as connection:
            ...
            for key, value in (("retention_days", str(retention_days)), ("retention_batch_size", str(batch_size)), ("retention_automatic", "1" if automatic else "0")):
                connection.execute("INSERT INTO control_plane_metadata(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", (key, value))
    ...
    def run_retention(self, correlation_id: str, automatic: bool = False) -> dict[str, int]:
        ...
        with connect(self.database_path) as connection:
            ...
            connection.execute("INSERT INTO control_plane_metadata(key,value) VALUES('retention_last_run_at',?) ON CONFLICT(key) DO UPDATE SET value=excluded.value", (now,))
    ...
    def authenticate(self, token: str) -> AuthenticatedIdentity:
        ...
        with connect(self.database_path) as connection:
            ...
            policy = json.loads(row["document_json"])
            actions = validate_actions(policy.get("allow", {}).get("control_plane_actions", []))
