...
TOKEN_PATTERN = re.compile(r"^acp_([0-9a-f]{24})_([A-Za-z0-9_-]{43})$")
...
def load_or_create_pepper(path: Path) -> bytes:
    configured = os.environ.get("AGENT_CONTROL_PLANE_CREDENTIAL_PEPPER_HEX")
...
def issue_credential(pepper: bytes) -> IssuedCredential:
    ...
    return IssuedCredential(
        ...
        token=f"acp_{credential_id}_{secret}",
