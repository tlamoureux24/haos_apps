...
from agent_control_plane.http_api import (
    ...
    admin_status,
    admin_home_assistant_status,
    admin_update_home_assistant,
    admin_test_home_assistant,
    admin_retry_home_assistant,
    create_event,
...
from agent_control_plane.home_assistant import deliver_batch
...
<section id="home-assistant" class="view"><div class="pagehead"><h1>Home Assistant</h1><p>Publiez des événements synthétiques dans Home Assistant par le proxy Supervisor.</p><span class="freshness" data-freshness="home-assistant"></span></div><div class="workspace"><section class="card"><div class="cardhead"><div><h2>Notifications</h2><p>Aucun jeton ni aucune URL ne doivent être saisis.</p></div><i id="ha-token-dot" class="status-dot disabled" role="img"></i></div><form id="ha-settings"><label class="permission"><input name="enabled" type="checkbox"><span>Activer les notifications Home Assistant<small>Désactivées par défaut.</small></span></label><fieldset><legend>Catégories publiées</legend><label class="permission"><input name="task_available" type="checkbox"><span>Nouvelle tâche disponible</span></label><label class="permission"><input name="task_completed" type="checkbox"><span>Tâche terminée</span></label><label class="permission"><input name="task_failed" type="checkbox"><span>Tâche échouée</span></label><label class="permission"><input name="technical_error" type="checkbox"><span>Incident technique</span></label></fieldset><button class="primary" type="submit">Enregistrer</button><p id="ha-settings-message" class="error"></p></form><div class="connector-actions"><button id="ha-test" class="secondary" type="button">Envoyer un événement de test</button><button id="ha-retry" class="secondary" type="button">Réessayer maintenant</button></div></section><section class="card"><div class="cardhead"><div><h2>État de livraison</h2><p id="ha-last-status">Chargement…</p></div></div><div id="ha-metrics" class="metrics"><article class="metric"><strong>–</strong><span>En attente</span></article><article class="metric"><strong>–</strong><span>En livraison</span></article><article class="metric"><strong>–</strong><span>Dead letters</span></article></div></section></div><section class="card audit-card"><div class="cardhead"><div><h2>Historique des livraisons</h2><p>Les 100 dernières livraisons réussies et toutes les livraisons non résolues.</p></div></div><div id="ha-history" class="tablewrap loading">Chargement…</div></section></section>
...
route_handlers = {
    ...
    "/admin/api/v1/home-assistant": admin_home_assistant_status,
    "/admin/api/v1/home-assistant/settings": admin_update_home_assistant,
    "/admin/api/v1/home-assistant/test": admin_test_home_assistant,
    "/admin/api/v1/home-assistant/retry": admin_retry_home_assistant,
    ...
    if settings.surface == "admin":
        ...
        async def schedule_loop():
            ...
            while True:
                ...
                await deliver_batch(control_plane, 5)
