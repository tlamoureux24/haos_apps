...
class Settings:
    ...
    def database_path(self) -> Path:
        return self.data_dir / "agent_control_plane.db"
...
def load_settings() -> Settings:
    ...
    ingress_proxy_ip = os.environ.get("AGENT_CONTROL_PLANE_INGRESS_PROXY_IP", "172.30.32.2")
    try:
        ipaddress.ip_address(ingress_proxy_ip)
    except ValueError as exc:
        raise RuntimeError("Invalid AGENT_CONTROL_PLANE_INGRESS_PROXY_IP") from exc
    try:
        intake_rate_limit = int(os.environ.get("AGENT_CONTROL_PLANE_INTAKE_RATE_LIMIT", "30"))
    except ValueError as exc:
        raise RuntimeError("Invalid AGENT_CONTROL_PLANE_INTAKE_RATE_LIMIT") from exc
    if not 1 <= intake_rate_limit <= 600:
        raise RuntimeError("AGENT_CONTROL_PLANE_INTAKE_RATE_LIMIT must be between 1 and 600")
    return Settings(
        data_dir=Path(os.environ.get("AGENT_CONTROL_PLANE_DATA_DIR", "/data")),
        surface=surface,
        log_level=log_level,
