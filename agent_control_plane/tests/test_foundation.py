...
from agent_control_plane.database import database_ready, initialize_database
...
from agent_control_plane.security import issue_credential, load_or_create_pepper, parse_and_verify_token
...
class InternationalizationTests(unittest.TestCase):
    def test_admin_interface_supports_french_and_english(self) -> None:
        ...
        self.assertIn("acp-language", ADMIN_JS)
...
class ConnectorContractTests(unittest.TestCase):
    ...
    def test_rejects_invalid_ingress_proxy_ip(self) -> None:
        with patch.dict(
            os.environ,
            {"AGENT_CONTROL_PLANE_INGRESS_PROXY_IP": "not-an-ip"},
            clear=True,
        ):
            with self.assertRaises(RuntimeError):
                load_settings()
    ...
    def test_accepts_configured_intake_rate_limit(self) -> None:
        with patch.dict(
            os.environ, {"AGENT_CONTROL_PLANE_INTAKE_RATE_LIMIT": "42"}, clear=True
        ):
            settings = load_settings()
    ...
    def test_rejects_out_of_range_intake_rate_limit(self) -> None:
        with patch.dict(
            os.environ, {"AGENT_CONTROL_PLANE_INTAKE_RATE_LIMIT": "0"}, clear=True
        ):
            with self.assertRaises(RuntimeError):
                load_settings()
...
class ConnectorDiscoveryVisibilityTests(unittest.TestCase):
    ...
    def _run(self, handler, request):
        with patch(
            "agent_control_plane.http_api.run_in_threadpool", new=self._inline_threadpool
        ):
    ...
    def _connector(self, root: Path) -> tuple[ControlPlane, str]:
        ...
        database_path = root / "control_plane.db"
    ...
    def test_schema_rejections_remain_precise_in_admin_api_and_persistence(self) -> None:
        ...
        for code in codes:
            with self.subTest(code=code), tempfile.TemporaryDirectory() as directory:
                ...
                with self.assertLogs("agent_control_plane.connectors", level="WARNING") as logs, patch(
                    "agent_control_plane.http_api.discover_streamable_http",
    ...
    def test_creation_and_reactivation_return_precise_schema_error(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with self.assertLogs("agent_control_plane.connectors", level="WARNING") as enable_logs, patch(
                "agent_control_plane.http_api.discover_streamable_http",
                new=AsyncMock(side_effect=rejection),
            ):
    ...
    def test_ordinary_edit_preserves_secret_and_name_only_skips_discovery(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "agent_control_plane.http_api.discover_streamable_http", new=AsyncMock()
            ) as discover:
...
class DatabaseReadinessTests(unittest.TestCase):
    ...
    def test_expected_schema_is_ready(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "control_plane.db"
    ...
    def test_old_generation_requires_clean_reinstall(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            initialize_database(path)
            with sqlite3.connect(path) as connection:
                connection.execute("UPDATE control_plane_metadata SET value='13' WHERE key='schema_generation'")
            with self.assertRaisesRegex(RuntimeError, "remove_app_data"):
                initialize_database(path)
            self.assertFalse(database_ready(path))
...
class CredentialTests(unittest.TestCase):
    def test_configured_pepper_avoids_persistent_file_access(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.dict(os.environ, {"AGENT_CONTROL_PLANE_CREDENTIAL_PEPPER_HEX": expected.hex()}):
...
class TaskCompositionTests(unittest.TestCase):
    def test_fixed_arguments_are_hidden_injected_and_independent_per_connector(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("agent_control_plane.fixed_arguments.connector_fernet", return_value=FakeFernet()):
                task_id = control_plane.create_task(
                    "Scoped management",
                    "scoped_management",
...
class AuditVerificationCheckpointTests(unittest.TestCase):
    ...
    def test_cockpit_and_retention_status_never_run_a_full_verification(self) -> None:
        http_source = (
            Path(__file__).resolve().parents[1] / "src" / "agent_control_plane" / "http_api.py"
        ).read_text(encoding="utf-8")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with sqlite3.connect(path) as connection:
                checkpoint_before = connection.execute(
                    "SELECT value FROM control_plane_metadata WHERE key='audit_valid_checkpoint'"
                ).fetchone()[0]
    ...
    def test_invalid_state_is_rechecked_at_the_full_verification_deadline(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with sqlite3.connect(path) as connection:
                ...
                connection.execute(
                    "UPDATE control_plane_metadata SET value=? WHERE key='audit_valid_checkpoint'",
